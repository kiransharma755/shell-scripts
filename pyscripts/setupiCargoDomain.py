...
def createWSQueue(queue_name,jndi_name):
   ...
   eaiResource = createGetJmsModule(jmsWSModuleName);
   if createUDD:
      jmsqueue = eaiResource.lookupDistributedQueue(queue_name)
   else:
      jmsqueue = eaiResource.lookupQueue(queue_name)
...
def applyJTASettings():
   ...
   if hasattr(cmo, 'TLOGWriteWhenDeterminerExistsEnabledx'):
      ...
      for det in cmo.getDeterminerCandidateResourceInfoList():
         if JTA_DETERMINERS.has_key(det.getDisplayName()) and not det.isDeterminer():
            cfgDeterminers.append(det.getDisplayName())
            print 'JTA determiner configured : ' + det.getDisplayName()
      cmo.setDeterminers(cfgDeterminers)
